...
class NetworkSettingsController extends AppController {
	...
	public function edit($id = null) {
		...
		if (file_exists($mac_file)) {
					...
					$mac_eth_file = '/sys/class/net/eth0/address';
					$mac_eth_addr = explode(':', file_get_contents($mac_eth_file));
					$network_setting['NetworkSetting']['node_name'] = 
					'RPi_Node' . hexdec($mac_eth_addr[5]);
...
";
		...
		str_replace(
			array('{wired_adapter_name}', '{wifi_protocol}', '{wifi_ip_address}', '{wifi_netmask}', '{wifi_mode}', '{wifi_channel}', '{wifi_ssid}', '{wired_mode}', '{wifi_adapter_name}',  '{lan_ip_address}', '{lan_netmask}'),
			array(
				...
				$network_setting['NetworkSetting']['lan_ip_address'],
				$network_setting['NetworkSetting']['lan_netmask']
			),
			$interfaces_conf);
