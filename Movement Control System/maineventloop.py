...
def distanceAccel():
    ...
    while True:
        ...
        if not xQ.empty() and not yQ.empty() and not zQ.empty():
            x = xQ.get()
            y = yQ.get()
            z = zQ.get()
            ...
            alist.append(mag)
            tlist.append(time_stamp)
            
            #print('(3): get queue data')
            if len(alist) == 3:
                dt = tlist[2]-tlist[1]
                #print ' the dt in a is ' +str(dt)
                out = float(alist[0]+alist[1])*(dt)*0.5
                vlist.append(out)
            #print('(4): calc vlist')
            #print('alist' + str(alist))
            if len(vlist) == 3 :
                out = float(vlist[0]+vlist[1])*(dt)*0.5
                dlist.append(out)
                #print ' the dt in v is ' +str(dt)
            #print('(5): calc dlist')
...
#Load the saved calibration profile
...
while True:
    ...
    if not isdestination and run_time > 20:
        ...
        if packet == None:
            ...
            print network.printPacket(packet)
            if flag == Mesh.FG_NONE:
                ...
                isdestination = True
                print ' (x,y) = ' + '(' + str(xdestination) + ',' + str(ydestination) + ')'
    ...
    #print 'isDestination: ' + str(TickAdd_AvgDistance5Thread) 
